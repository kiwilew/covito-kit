...
public class TrackerClient {
	...
	public StorageServer getStoreStorage(TrackerServer trackerServer,
		...
		try {
			...
			ip_addr = new String(pkgInfo.body,
	...
	public StorageServer[] getStoreStorages(TrackerServer trackerServer,
		...
		try {
			...
			int ipPortLen = pkgInfo.body.length
					- (ProtoCommon.FDFS_GROUP_NAME_MAX_LEN + 1);
			final int recordLength = ProtoCommon.FDFS_IPADDR_SIZE - 1
			...
			for (int i = 0; i < serverCount; i++) {
				ip_addr = new String(pkgInfo.body, offset,
						ProtoCommon.FDFS_IPADDR_SIZE - 1).trim();
	...
	public StorageServer getFetchStorage(TrackerServer trackerServer,
		...
		ServerInfo[] servers = this.getStorages(trackerServer,
				ProtoCommon.TRACKER_PROTO_CMD_SERVICE_QUERY_FETCH_ONE,
				groupName, filename);
		if (servers == null) {
			return null;
		} else {
			return new StorageServer(servers[0].getIpAddr(),
					servers[0].getPort(), 0);
		}
	}
	...
	public StorageServer getUpdateStorage(TrackerServer trackerServer,
			String groupName, String filename) throws IOException {
		ServerInfo[] servers = this.getStorages(trackerServer,
				ProtoCommon.TRACKER_PROTO_CMD_SERVICE_QUERY_UPDATE, groupName,
				filename);
		if (servers == null) {
			return null;
		} else {
	...
	public ServerInfo[] getFetchStorages(TrackerServer trackerServer,
			String groupName, String filename) throws IOException {
		return this.getStorages(trackerServer,
	...
	protected ServerInfo[] getStorages(TrackerServer trackerServer, byte cmd,
		...
		try {
			...
			header = ProtoCommon.packHeader(cmd,
					ProtoCommon.FDFS_GROUP_NAME_MAX_LEN + bFileName.length,
					(byte) 0);
			...
			for (int i = 1; i < server_count; i++) {
				servers[i] = new ServerInfo(new String(pkgInfo.body, offset,
						ProtoCommon.FDFS_IPADDR_SIZE - 1).trim(), port);
				offset += ProtoCommon.FDFS_IPADDR_SIZE - 1;
			}
	...
	public StorageServer getFetchStorage1(TrackerServer trackerServer,
			String file_id) throws IOException {
	...
	public StructGroupStat[] listGroups(TrackerServer trackerServer)
		...
		try {
			header = ProtoCommon.packHeader(
					ProtoCommon.TRACKER_PROTO_CMD_SERVER_LIST_GROUP, 0,
					(byte) 0);
			out.write(header);
			...
			return decoder.decode(pkgInfo.body, StructGroupStat.class,
					StructGroupStat.getFieldsTotalSize());
		} catch (IOException ex) {
			if (!bNewConnection) {
				try {
					trackerServer.close();
	...
	public StructStorageStat[] listStorages(TrackerServer trackerServer,
		...
		try {
			...
			System.arraycopy(bGroupName, 0, wholePkg, header.length,
					bGroupName.length);
			if (ipAddrLen > 0) {
				System.arraycopy(bIpAddr, 0, wholePkg, header.length
						+ bGroupName.length, ipAddrLen);
			}
			out.write(wholePkg);
	...
	public boolean deleteStorage(String groupName, String storageIpAddr)
			throws IOException {
		return this.deleteStorage(ClientGlobal.g_tracker_group, groupName,
				storageIpAddr);
	}
	...
	public boolean deleteStorage(TrackerGroup trackerGroup, String groupName,
			String storageIpAddr) throws IOException {
		int serverIndex;
		int notFoundCount;
		...
		for (serverIndex = 0; serverIndex < trackerGroup.tracker_servers.length; serverIndex++) {
			...
			try {
				StructStorageStat[] storageStats = listStorages(trackerServer,
						groupName, storageIpAddr);
		...
		for (serverIndex = 0; serverIndex < trackerGroup.tracker_servers.length; serverIndex++) {
			try {
				...
				System.err.println("connect to server "
						+ trackerGroup.tracker_servers[serverIndex]
			...
			try {
				if (!this
						.deleteStorage(trackerServer, groupName, storageIpAddr)) {
					if (this.errno != 0) {
						if (this.errno == ProtoCommon.ERR_NO_ENOENT) {
							notFoundCount++;
